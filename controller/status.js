...
export const fetchStatus = (req, res) => {
    const q = "SELECT s.*, s.id AS statusId, u.name, u.profile_picture, u.id AS userId FROM status s JOIN users u ON s.status_user_id = u.id LEFT JOIN relationship r ON r.followed_id = u.id WHERE r.follower_id = ? OR u.id = ? ORDER BY s.created_at DESC";
...
export const Status = (req, res) => {
    const q = "INSERT INTO status (image, created_at, status_user_id) VALUES (?, NOW(), ?)";
